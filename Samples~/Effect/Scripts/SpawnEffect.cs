...
public class SpawnEffect : Effect
{
    ...
    [SerializeField]
    private TweenSettings<Vector3> scaleSettings = new();

    [SerializeField]
    private ShakeSettings shakeSettings = new();

    [SerializeField]
    private TweenSettings<float> flashInSettings = new() { startFromCurrent = true };

    [SerializeField]
    private TweenSettings<float> flashOutSettings = new() { startFromCurrent = true };
    ...
    private async Task Flash(Renderer[] renderers)
    {
        ...
        foreach (Renderer renderer in renderers)
            tasks.Add(FlashRenderer(renderer));
