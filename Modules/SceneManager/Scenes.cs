...
namespace CookieUtils.Extras.SceneManager
{
    ...
    public static class Scenes
    {
        ...
        private static async void Initialize()
        {
            ...
            if (_settings.bootstrapScene.UnsafeReason != SceneReferenceUnsafeReason.Empty)
            {
                if (!IsSceneLoaded(_settings.bootstrapScene))
                    ...
                    Debug.Log(
                        "[CookieUtils.Extras.SceneManager] Bootstrap scene already loaded, skipping"
                    );
        ...
        private static void FindSceneTransition()
        {
            ...
            Debug.Log(
                "[CookieUtils.Extras.SceneManager] Found scene transition in bootstrap scene"
            );
        ...
        public static async Task UnloadAllScenes()
        {
            ...
            if (unloadedScenes > 0)
                Debug.Log($"[CookieUtils.Extras.SceneManager] Unloaded {unloadedScenes} scenes");
        }
